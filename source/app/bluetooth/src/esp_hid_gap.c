...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    uint8_t manufacturer_data[] = {0xE5, 0x02}; // Example: Apple (0x004C)

    uint8_t service_data[] = {
    0x12, 0x18,       // HID Service UUID (16-bit, little-endian)
    0x01              // Device type (e.g., 0x01 for mouse, custom-defined)
};

    esp_ble_adv_data_t ble_adv_data = {
        .set_scan_rsp = false,                     // Advertising packet
        .include_name = false,                     // Name in scan response instead
        .include_txpower = true,                   // Include TX power
        .min_interval = 0x0006,                    // Connection interval
        .max_interval = 0x0010,                    // Connection interval
        .appearance = appearance,                  // Appearance (HID Mouse)
        .manufacturer_len = 0,  // Manufacturer data
        .p_manufacturer_data = NULL,  // Manufacturer data pointer
        .service_data_len = 0,                     // Service data in scan response
        .p_service_data = NULL,
        .service_uuid_len = sizeof(hidd_service_uuid128), // Include UUID
        .p_service_uuid = (uint8_t *)hidd_service_uuid128,
        .flag = ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT // Required flags
    };

    esp_ble_adv_data_t ble_scan_rsp_data = {
        .set_scan_rsp = true,                      // Scan response packet
        .include_name = true,                      // Include full device name here
        .include_txpower = false,                  // Already in advertising packet
        .manufacturer_len = sizeof(manufacturer_data),                     // Manufacturer data in adv only
        .p_manufacturer_data = (uint8_t*)manufacturer_data,
        .service_data_len = 0,  // Include service data
        .p_service_data = NULL,
        .service_uuid_len = 0,                     // UUID already in adv packet
        .p_service_uuid = NULL,
        .flag = 0                                  // Flags not allowed in scan response
    };
